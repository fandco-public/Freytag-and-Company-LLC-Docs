...
APfMS's many fields and choices can be saved and reloaded later.  Even partial work can be saved, reloaded, changed and re-saved.  This means even if not everything is ready for publication at the same time, you can preserve your workflow and submit when ready.  
...
4. Showing app price in each country’s currency using the market rates generated and updated weekly by APfMS.
...
APfMS needs these files in one folder together.  Use the folder picker opened with the "Browse..." button (see #1 below) to choose this folder and load the image file(s) and application binary.  The "Item Type" (see #2 below) and "File Name" (see #3 below) columns are blank until APfMS finds the files needed.  
